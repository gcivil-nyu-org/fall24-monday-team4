...
class LocationViewsTest(TestCase):
    ...
    def test_handle_match_request(self):
        ...
        response = self.client.post(reverse('handle_match_request'), {
            'match_id': match.id,
            'action': 'accept'
        })
        self.assertEqual(response.status_code, 302)
    ...
    def test_create_trip(self):
        data = {
            'planned_departure': (datetime.now() + timedelta(hours=1)).strftime("%Y-%m-%dT%H:%M"),
            'start_latitude': '40.7128',
            'start_longitude': '-74.0060',
            'dest_latitude': '40.7580',
            'dest_longitude': '-73.9855',
    ...
    def test_send_match_request(self):
        ...
        response = self.client.post(reverse('send_match_request'), {
            'trip_id': other_trip.id,
            'action': 'request'
        })
        self.assertEqual(response.status_code, 200)
        self.assertTrue(Match.objects.filter(trip1=self.trip, trip2=other_trip).exists())
    ...
    def test_send_match_request_cancel(self):
        ...
        self.assertEqual(response.status_code, 200)
        self.assertFalse(Match.objects.filter(trip1=self.trip, trip2=other_trip).exists())
